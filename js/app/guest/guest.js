...
export const guest = (() => {
  ...
  const showGuestName = () => {
    ...
    if (name) {
      ...
      div.innerHTML = `
                <p class="mt-0 mb-1 mx-0 p-0" style="font-size: 0.95rem;">${guestName?.getAttribute(
                  "data-message"
                )}</p>
  ...
  const buildGoogleCalendar = () => {
    ...
    const data = {
      action: "TEMPLATE",
      text: "The Wedding of Agus Zamroni and Alip Dea Rizky Fany",
      dates: "2025-04-20 10:00:00/2025-04-20 11:00:00",
      details:
    ...
    data.dates = `${formatDate(data.dates.split("/")[0])}/${formatDate(
      data.dates.split("/")[1]
    )}`;
